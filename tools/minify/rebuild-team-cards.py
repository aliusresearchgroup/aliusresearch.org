...
/* --- Click-to-expand: FLIP-based genuine grid reflow ---
   Clicked card takes 2x2 grid cells (real layout change → it's a square
   if rows are equal to columns). Siblings actually reflow around it.
   JS applies FLIP (First-Last-Invert-Play) inverse transforms so the
   transition between the two grid states is smooth and synchronized. */

body.wsite-page-team .team-card {
  cursor: pointer;
  transform-origin: top left;
  will-change: transform, max-height;
  /* Base transition for max-height + opacity. FLIP injects its own
     transform transition inline so it can enable/disable per phase. */
  transition: transform 1600ms cubic-bezier(0.22, 0.61, 0.36, 1),
              max-height 1600ms cubic-bezier(0.22, 0.61, 0.36, 1),
              opacity 1600ms ease,
              border-color 1600ms ease,
              box-shadow 1600ms ease;
}

/* Equal row heights so a 2×2 span produces a symmetrical square
...
/* Clicked card: real 2×2 grid occupation + enforced 1:1 aspect ratio */
body.wsite-page-team .team-card--expanded {
  ...
  box-shadow: 0 14px 40px -10px rgba(26, 77, 46, 0.25);
}
...
/* Siblings: they reflow naturally — FLIP handles the motion. Just a
   subtle opacity dim as an affordance. */
body.wsite-page-team .team-grid--has-expanded .team-card:not(.team-card--expanded) {
  opacity: 0.7;
}

@media (max-width: 520px) {
  /* Two columns won't fit comfortably — fall back to natural single-column
     accordion: card stays 1×1 in the grid, bio still un-clamps via max-height */
  body.wsite-page-team .team-card--expanded {
    grid-column: auto;
    grid-row: auto;
    aspect-ratio: auto;
