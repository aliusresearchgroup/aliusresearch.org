...
/* Grid — explicit track counts (not auto-fill) so grid-template-columns /
   grid-template-rows can be natively animated when we emphasise one cell. */
body.wsite-page-team .team-grid {
  ...
  margin: 0 auto;
  padding: 24px;
  transition: grid-template-columns 1600ms cubic-bezier(0.22, 0.61, 0.36, 1),
              grid-template-rows 1600ms cubic-bezier(0.22, 0.61, 0.36, 1),
              gap 600ms ease;
}
@media (max-width: 1024px) {
  body.wsite-page-team .team-grid { grid-template-columns: repeat(3, 1fr); }
}
@media (max-width: 700px) {
  body.wsite-page-team .team-grid { grid-template-columns: repeat(2, 1fr); }
}
...
/* Clicked card: only cosmetic cues (border, shadow). The actual geometric
   growth happens at the grid-track level via inline styles from JS. */
